...
class LSFJob(Job):
    ...
    def prepare_header(queue_list, mem_gb, time_hours, cores, JOB_PATHS, n=1):
        ...
        lines = [
            ...
            "#BSUB -J job[1-%d]" % n,
            "",
            "#BSUB -o %s/$JOB_NAME.$JOB_ID.$TASK_ID.eo" % JOB_PATHS["stdeo_path"],
        ]
...
class SGEJob(Job):
    ...
    def prepare_header(queue_list, mem_gb, time_hours, cores, JOB_PATHS, n=1):
        ...
        lines = [
            ...
            "#$ -e %s/$JOB_NAME.$JOB_ID.$TASK_ID.e" % JOB_PATHS["stdeo_path"],
            "#$ -o %s/$JOB_NAME.$JOB_ID.$TASK_ID.o" % JOB_PATHS["stdeo_path"],
            "#$ -j y",
        ]
